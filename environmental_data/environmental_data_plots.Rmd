...
for coral outplant figure -- get max daily temp experienced for time points of coral monitoring
```{r}
#december 6 (initial)
#jan 26 (t1)
#april 4 (t2)
...
```{r}
  ...
  MMM_mac <- raster::extract(MMM.data, extract.pts_macn,method="simple") #29.55
  MMM_emerald <- raster::extract(MMM.data, extract_em,method="simple") #29.53
...
### MMM from 2022-2023 CRW

manually downloaded Dec 1-31 2022 data from: https://ncei.noaa.gov/data/oceans/crw/5km/v3.1/nc/v1.0/daily/sst/2022/

```{r}
  # read all the .nc files 
    flist_2022<-list.files("~/Downloads/coraltemp_v3.1_2022",pattern="*.nc",full.names=TRUE)
  # i deleted any .nc files that were causing issues with importing (two files)
      
  # brick them 
    ras_2022<-lapply(flist_2022, stack)
...
# macNorth
  ...
  ## Extract data by coordinates 
    ext_MacNorth <- lapply(ras_2022,raster::extract,extract.pts_macn)
    head(ext_MacNorth)
    MacNorth_SST_2022 <- unlist(ext_MacNorth, recursive = FALSE)
    head(MacNorth_SST_2022)
    
  ## Extract data by coordinates 
    ext_Emerald <- lapply(ras_2022,raster::extract,extract_em)
    head(ext_Emerald)
    Emerald_SST_2022 <- unlist(ext_Emerald, recursive = FALSE)
    head(Emerald_SST_2022)
```

2023 data: 
run in terminal 
```{bash}
wget -r -np -nH --cut-dirs=7 --reject="index.html*" https://www.ncei.noaa.gov/data/oceans/crw/5km/v3.1/nc/v1.0/daily/sst/2023/
```

```{r}
  # read all the .nc files 
    flist_2023<-list.files("~/Downloads/daily/sst/2023",pattern="*.nc",full.names=TRUE)
  # i deleted any .nc files that were causing issues with importing (two files)
      
  # brick them 
    ras_2023<-lapply(flist_2023, stack)
...
# macNorth
  ...
  ## Extract data by coordinates 
    ext_MacNorth <- lapply(ras_2023,raster::extract,extract.pts_macn)
    head(ext_MacNorth)
    MacNorth_SST_2023 <- unlist(ext_MacNorth, recursive = FALSE)
    head(MacNorth_SST_2023)
    
  ## Extract data by coordinates 
    ext_Emerald <- lapply(ras_2023,raster::extract,extract_em)
    head(ext_Emerald)
    Emerald_SST_2023 <- unlist(ext_Emerald, recursive = FALSE)
    head(Emerald_SST_2023)
    
 save(ext_Emerald, ext_MacNorth, Emerald_SST_2022, MacNorth_SST_2022, Emerald_SST_2023, MacNorth_SST_2023, file = "combined_rasters.Rdata")
```


```{r}
# Create a date list 
    Dates_2022<-seq.Date(as.Date("2022-12-01"), as.Date("2022-12-31"), by = "day")
    Dates_2022<-as.character(Dates_2022)
      
# Bind GPS points and Dates    Date.location <- merge(pts, Date)
    Emerald_SST_2022.data<-data.frame(Dates_2022, Emerald_SST_2022)
    Emerald_SST_2022.data$Date<-as.Date(Emerald_SST_2022.data$Dates_2022)
    head(Emerald_SST_2022.data)  
    Emerald_SST_2022.data %>% 
      rename(Temperature = Emerald_SST_2022) %>% 
      mutate(site = "Emerald") ->Emerald_SST_2022.data
    
    MacNorth_SST_2022.data <- data.frame(Dates_2022, MacNorth_SST_2022)
    MacNorth_SST_2022.data$Date <- as.Date(MacNorth_SST_2022.data$Dates_2022)
    
    MacNorth_SST_2022.data %>% 
      rename(Temperature = MacNorth_SST_2022) %>% 
      mutate(site = "MacNorth") -> MacNorth_SST_2022.data
    
    Dates_2023<-seq.Date(as.Date("2023-01-01"), as.Date("2023-12-31"), by = "day")
    Dates_2023<-as.character(Dates_2023)
    #head(Date)
    
    Emerald_SST_2023.data<-data.frame(Dates_2023, Emerald_SST_2023)
    Emerald_SST_2023.data$Date<-as.Date(Emerald_SST_2023.data$Dates_2023)
    head(Emerald_SST_2023.data)  
    Emerald_SST_2023.data %>% 
      rename(Temperature = Emerald_SST_2023) %>% 
      mutate(site = "Emerald") ->Emerald_SST_2023.data
    
    MacNorth_SST_2023.data <- data.frame(Dates_2023, MacNorth_SST_2023)
    MacNorth_SST_2023.data$Date <- as.Date(MacNorth_SST_2023.data$Dates_2023)
    
    MacNorth_SST_2023.data %>% 
      rename(Temperature = MacNorth_SST_2023) %>% 
      mutate(site = "MacNorth") -> MacNorth_SST_2023.data
    
  rbind(Emerald_SST_2022.data, MacNorth_SST_2022.data) %>% 
    dplyr::select(!Dates_2022) %>% 
    full_join(., Emerald_SST_2023.data) %>% 
    full_join(., MacNorth_SST_2023.data) %>% 
    dplyr::select(!Dates_2023) -> SST_2022_2023
  
#write_csv(SST_2022_2023, "SST_2022_2023.csv")
```

temp summary for SST 2022-2023
```{r}
SST_2022_2023
temp.summary<- summaryBy(Temperature ~ site, data = SST_2022_2023,
...
```{r}
SST_2022_2023$Month <- month(SST_2022_2023$Date)
SST_2022_2023$Year <- year(SST_2022_2023$Date)

# Calculate month-year mean SST

MM.CRW_SST_2022_2023 <- SST_2022_2023 %>% 
  group_by(Month, Year, site) %>% 
summarize(MM = mean(Temperature, na.rm = TRUE))

# Calculate monthly mean SST

monthlyclimatology<-MM.CRW_SST_2022_2023 %>% 
  group_by(Month, site) %>%
    summarize(Clima_M = mean(MM, na.rm = TRUE))

monthlyclimatology %>% 
  filter(site == "Emerald") -> monthlyclimatology_emerald

monthlyclimatology %>% 
  filter(site == "MacNorth") -> monthlyclimatology_MacNorth

# Get maximum monthly mean (MMM)
  MMM_SST_2022_2023_Emerald <-max(monthlyclimatology_emerald$Clima_M) #30.99
  MMM_SST_2022_2023_MacNorth <- max(monthlyclimatology_MacNorth$Clima_M) #30.93
```

### MMM from in-situ data
```{r}
str_all %>% 
  filter(Date >= "2022-12-06") %>% 
  full_join(ctd_emerald_temp_sept9) -> tempdata

# Extract months and years from date
  tempdata$Month<-month(tempdata$Date)
  tempdata$Year<-year(tempdata$Date)

# Calculate month-year mean SST
  tempdata.MM_insitu<-tempdata %>%
    group_by(Month, Year, reef) %>%
    summarize(MM = mean(Temperature, na.rm = TRUE))

# Calculate monthly mean SST
  insitu.MonthlyClimatology<-tempdata.MM_insitu %>%
    group_by(Month,reef) %>%
    summarize(Clima_M = mean(MM, na.rm = TRUE))

# Get maximum monthly mean (MMM)
  insitu.MonthlyClimatology %>% 
    filter(reef == "Emerald") -> Emerald_insitu.MonthlyClimatology
  
    insitu.MonthlyClimatology %>% 
    filter(reef == "MacNorth") -> MacNorth_insitu.MonthlyClimatology
  
  MMM_Emerald_insitu<-max(Emerald_insitu.MonthlyClimatology$Clima_M) #30.74
  MMM_MacNorth_insitu <-max(MacNorth_insitu.MonthlyClimatology$Clima_M) #31.51
```

### plot all daily temps together
```{r}
SST_2022_2023 %>% 
  rename(mean_daily_temp = Temperature) -> SST_2022_2023_edited

SST_2022_2023_edited %>% 
  mutate(site = case_when(site == "Emerald" ~ "Emerald_SST", 
                          site == "MacNorth" ~ "MacNorth_SST")) -> SST_2022_2023_edited

tempdata %>% 
  mutate(reef = case_when(reef == "Emerald" ~ "Emerald_insitu", 
                          reef == "MacNorth" ~ "MacNorth_insitu")) -> tempdata_edited


tempdata_edited %>% 
  dplyr::rename(site = reef) %>% 
  group_by(Date, site) %>% 
  mutate(mean_daily_temp = mean(Temperature)) %>% 
  full_join(., SST_2022_2023_edited) %>% 
  dplyr::select(!c(Month, Year, Temperature, Time)) %>%
  distinct() %>% 
  filter(Date < "2023-12-01") %>% 
  ggplot(., aes(x=Date, y=mean_daily_temp, color = site)) +
  geom_line() +
  theme_bw() +
  labs(x="Date", y = "Daily Temp (ºC)")  + 
  scale_color_manual(values = c("Emerald_insitu" = "#edb9c3", "MacNorth_insitu" = "#5370c9", "Emerald_SST" = "tan", "MacNorth_SST" = "mediumpurple")) + 
  geom_hline(yintercept = 29.5, linetype = "dashed", color = "grey") +
  geom_hline(yintercept = 30.74, linetype = "dashed", color = "#edb9c3") +
  geom_hline(yintercept = 31.51, linetype = "dashed", color = "#5370c9") +
  geom_hline(yintercept = 30.9, linetype = "dashed", color = "mediumpurple") 
#ggsave("alldailytemps.pdf", width = 10, height = 7)
  # annotate("text", x = max(tempdata_edited$Date) + 1, y = 29.3, label = "SST 1981-2012 MMM", hjust = 1.1, color = "grey") +
  # annotate("text", x = max(tempdata_edited$Date) + 1, y = 30.3, label = "Emerald in-situ MMM", hjust = 1.1, color = "#edb9c3") +
  # annotate("text", x = max(tempdata_edited$Date) + 1, y = 31.3, label = "MacNorth in-situ MMM", hjust = 1.1, color = "#5370c9") +
  # annotate("text", x = max(tempdata_edited$Date) + 2, y = 31, label = "SST 2022-2023 MMM", hjust = 1.1, color = "mediumpurple") 

```


### insitu temp summary for Site 
```{r}
str_all %>% 
  filter(Date >= "2022-12-06") %>% 
  full_join(ctd_emerald_temp_sept9) %>% 
...
str_all %>% 
  ...
  dplyr::select(!Time) %>% 
  group_by(Date, reef) %>% 
  mutate(mean_daily_temp = mean(Temperature)) -> insitu_temp_data

temp.summary_insitu<- summaryBy(mean_daily_temp ~ reef, data = insitu_temp_data,
                         FUN = function(x) { c(max = max(x), min = min(x)) } )

temp.summary_insitu$Temp.seasonal <- temp.summary_insitu$mean_daily_temp.max - temp.summary_insitu$mean_daily_temp.min
...
all_temp_data %>% 
  mutate(Season = case_when(`Datetime (EST)` < "2023-05-26 00:00:00" ~ "Dry",
                            ...
                            `Datetime (EST)` > "2023-10-15 00:00:00" ~ "Dry")) -> all_temp_data
```

### DHW from 1981-2012 CRW
```{r}
all_temp_data %>% 
  ...
  mutate(Weeks_Stress = Stress/7)  -> Long.data.DHW_1981CRW

Long.data.DHW_1981CRW %>% 
  filter(reef == "Emerald") %>% 
  mutate(reef = case_when(reef == "Emerald" ~ "Emerald_1981CRW")) -> Long.data.DHW_1981CRW_emerald

Long.data.DHW_1981CRW_emerald$DHW<-rollapplyr(Long.data.DHW_1981CRW_emerald$Weeks_Stress,list(-(83:0)),sum,fill=NA)

Long.data.DHW_1981CRW %>% 
  filter(reef == "MacNorth") %>% 
  mutate(reef = case_when(reef == "MacNorth" ~ "MacNorth_1981CRW")) -> Long.data.DHW_1981CRW_macnorth

Long.data.DHW_1981CRW_macnorth$DHW<-rollapplyr(Long.data.DHW_1981CRW_macnorth$Weeks_Stress,list(-(83:0)),sum,fill=NA)

rbind(Long.data.DHW_1981CRW_emerald, Long.data.DHW_1981CRW_macnorth) %>% 
ggplot(., aes(x=Date)) + 
...
rbind(Long.data.DHW_1981CRW_emerald, Long.data.DHW_1981CRW_macnorth) %>% 
          ...
          rename(maxDHW=max) -> MaxDHW_1981CRW

```

### DHW from 2022-2023 CRW
```{r}
all_temp_data %>% 
  dplyr::group_by(Date, reef) %>% 
  mutate(mean_daily_temp = mean(Temperature)) %>% 
  ...
  dplyr::select(!c("Temperature","Datetime (EST)")) %>% 
  distinct() %>% 
  mutate(HotSpot = case_when(reef == "MacNorth" ~ (mean_daily_temp - MMM_SST_2022_2023_MacNorth),
                             reef == "Emerald" ~ (mean_daily_temp - MMM_SST_2022_2023_Emerald))) %>% 
  mutate(HotSpot = ifelse(HotSpot >= 0, HotSpot, 0)) %>% 
  mutate(Stress = ifelse(HotSpot >= 1, HotSpot, 0)) %>% 
  mutate(Weeks_Stress = Stress/7)  -> Long.data.DHW_2022CRW

Long.data.DHW_2022CRW %>% 
  filter(reef == "Emerald") %>% 
  mutate(reef = case_when(reef == "Emerald" ~ "Emerald_2022CRW")) -> Long.data.DHW_2022CRW_Emerald

Long.data.DHW_2022CRW_Emerald$DHW<-rollapplyr(Long.data.DHW_2022CRW_Emerald$Weeks_Stress,list(-(83:0)),sum,fill=NA)

Long.data.DHW_2022CRW %>% 
  filter(reef == "MacNorth") %>% 
  mutate(reef = case_when(reef == "MacNorth" ~ "MacNorth_2022CRW")) -> Long.data.DHW_2022CRW_MacNorth

Long.data.DHW_2022CRW_MacNorth$DHW<-rollapplyr(Long.data.DHW_2022CRW_MacNorth$Weeks_Stress,list(-(83:0)),sum,fill=NA)

rbind(Long.data.DHW_2022CRW_Emerald, Long.data.DHW_2022CRW_MacNorth) %>% 
ggplot(., aes(x=Date)) + 
    geom_hline(yintercept=8, linetype="dashed")+
    geom_hline(yintercept=4, linetype="dashed")+
    geom_line(aes(y = DHW, color = reef)) +
...
rbind(Long.data.DHW_2022CRW_Emerald, Long.data.DHW_2022CRW_MacNorth) %>% 
          group_by(reef) %>%
          summarize(max = max(DHW, na.rm = TRUE)) %>% 
          rename(maxDHW=max) -> MaxDHW_SST2022

MaxDHW_SST2022
```

### DHW from insitu data
```{r}
all_temp_data %>% 
  dplyr::group_by(Date, reef) %>% 
  mutate(mean_daily_temp = mean(Temperature)) %>% 
  ...
  dplyr::select(!c("Temperature","Datetime (EST)")) %>% 
  distinct() %>% 
  mutate(HotSpot = case_when(reef == "MacNorth" ~ (mean_daily_temp - MMM_MacNorth_insitu),
                             reef == "Emerald" ~ (mean_daily_temp - MMM_Emerald_insitu))) %>% 
  mutate(HotSpot = ifelse(HotSpot >= 0, HotSpot, 0)) %>% 
  mutate(Stress = ifelse(HotSpot >= 1, HotSpot, 0)) %>% 
  mutate(Weeks_Stress = Stress/7)  -> Long.data.DHW_insitu

Long.data.DHW_insitu %>% 
  filter(reef == "Emerald") %>% 
  mutate(reef = case_when(reef == "Emerald" ~ "Emerald_insitu")) -> Long.data.DHW_insitu_emerald

Long.data.DHW_insitu_emerald$DHW<-rollapplyr(Long.data.DHW_insitu_emerald$Weeks_Stress,list(-(83:0)),sum,fill=NA)

Long.data.DHW_insitu %>% 
  filter(reef == "MacNorth") %>% 
  mutate(reef = case_when(reef == "MacNorth" ~ "MacNorth_insitu")) -> Long.data.DHW_insitu_macnorth

Long.data.DHW_insitu_macnorth$DHW<-rollapplyr(Long.data.DHW_insitu_macnorth$Weeks_Stress,list(-(83:0)),sum,fill=NA)

rbind(Long.data.DHW_insitu_emerald, Long.data.DHW_insitu_macnorth) %>% 
ggplot(., aes(x=Date)) + 
    geom_hline(yintercept=8, linetype="dashed")+
    geom_hline(yintercept=4, linetype="dashed")+
    geom_line(aes(y = DHW, color = reef)) +
...
rbind(Long.data.DHW_insitu_emerald, Long.data.DHW_insitu_macnorth) %>% 
          group_by(reef) %>%
          summarize(max = max(DHW, na.rm = TRUE)) %>% 
          rename(maxDHW=max) -> MaxDHW_insitu
...
```{r}
rbind(Long.data.DHW_1981CRW_emerald, Long.data.DHW_1981CRW_macnorth,
      Long.data.DHW_2022CRW_Emerald, Long.data.DHW_2022CRW_MacNorth,
      Long.data.DHW_insitu_emerald, Long.data.DHW_insitu_macnorth) %>% 
  mutate(site = factor(reef, levels = c("MacNorth_1981CRW", "Emerald_1981CRW", "MacNorth_2022CRW", "MacNorth_insitu", "Emerald_insitu", "Emerald_2022CRW"))) %>% 
  ggplot(., aes(x=ymd(Date), y=DHW, color = reef)) + 
  geom_line() + 
    geom_hline(yintercept=8, linetype="dashed")+
    geom_hline(yintercept=4, linetype="dashed") +
  theme_bw()
...
## radar plot with all DHWs

```{r}
SST_2022_2023
temp.summary_SST_2022_2023<- summaryBy(mean_daily_temp ~ site, data = SST_2022_2023,
                         FUN = function(x) { c(max = max(x), min = min(x)) } )

temp.summary_SST_2022_2023$Temp.seasonal <- temp.summary_SST_2022_2023$mean_daily_temp.max - temp.summary_SST_2022_2023$mean_daily_temp.min

temp.summary_SST_2022_2023 %>% 
  rename(reef = site) %>% 
  mutate(reef = case_when(reef == "Emerald" ~ "Emerald_2022CRW",
                          reef == "MacNorth" ~ "MacNorth_2022CRW")) -> temp.summary_SST_2022_2023
...
temp.summary_insitu %>% 
  mutate(reef = case_when(reef == "Emerald" ~ "Emerald_insitu",
                          reef == "MacNorth" ~ "MacNorth_insitu")) -> temp.summary_insitu
...
#bind all DHWs
max_DHW_alltypes %>% 
  filter(reef != "Emerald_1981CRW" & reef != "MacNorth_1981CRW")  -> DHW

rbind(temp.summary_SST_2022_2023, temp.summary_insitu) %>% 
full_join(., DHW) -> temp_DHW_summary
...
# Color vector
colors_border=c("tan", "mediumpurple", "#edb9c3", "#5370c9")
colors_in=c("tan", "mediumpurple","#edb9c3", "#5370c9")

# Define colors with transparency using adjustcolor
colors_border <- c(adjustcolor("tan", alpha = 0.9), adjustcolor("mediumpurple", alpha = 0.9), adjustcolor("#edb9c3", alpha = 0.9), adjustcolor("#5370c9", alpha = 0.9))
colors_in <- c(adjustcolor("tan", alpha = 0.4), adjustcolor("mediumpurple", alpha = 0.4), adjustcolor("#edb9c3", alpha = 0.4), adjustcolor("#5370c9", alpha = 0.4))
...
all_temp_data$reef <- as.factor(all_temp_data$reef)
all_temp_data$Season <- as.factor(all_temp_data$Season)

all_temp_data %>% 
  mutate(temp_cub = (Temperature)^3) -> all_temp_data

treat_model <- lm(temp_cub ~ reef*Season, data = all_temp_data)
treat_model_metrics <- augment(treat_model)
plot(treat_model) #that looks much better


# Run the two-way ANOVA
result <- aov(temp_cub ~ reef * Season, data = all_temp_data)
...
TukeyHSD(aov(temp_cub ~ reef * Season, data = all_temp_data))
capture.output(TukeyHSD(aov(temp_cub ~ reef * Season, data = all_temp_data)), file = "temp_anova_tukey.csv")
```
...
rbind(ecopar_emerald_clean, ecopar_macnorth_clean) %>% 
  ...
  dplyr::mutate(interval = floor_date(`Datetime (EST)`, unit = "30 minutes")) %>% 
  dplyr::mutate(hour = hour(interval), Date =as.Date(interval) ) %>% 
  ungroup() %>% 
  filter(hour >= 10 & hour <= 15) %>% 
  dplyr::group_by(Date, reef) %>% 
  dplyr::summarise(daily_dose = sum(`PAR(avg)`)) %>% 
  dplyr::mutate(PAR_mol_day = daily_dose/1000) -> PAR_summary

PAR_summary %>% 
  group_by(reef) %>% 
  filter(Date <= "2023-07-11") %>% 
  summarise(mean_PAR_day = mean(PAR_mol_day), count = n(), sd = sd(PAR_mol_day))
...
rbind(ecopar_emerald_clean, ecopar_macnorth_clean) %>% 
  ...
  mutate(Season = case_when(Date < "2023-05-26" ~ "Dry",
                            Date >= "2023-05-26" ~ "Wet")) %>% 
  filter(hour >= 10 & hour <= 15) %>% 
...
```{r}
PAR_summary %>% 
filter(Date <= "2023-07-11") -> PAR_summary_subset

PAR_summary_subset %>% 
  mutate(Season = case_when(Date < "2023-05-26" ~ "Dry",
                            Date >= "2023-05-26" & Date <= "2023-10-15" ~ "Wet",
                            Date > "2023-10-15" ~ "Dry")) -> PAR_summary_subset

library(rstatix)
# Ensure 'site' and 'season' are factors
PAR_summary_subset$reef <- as.factor(PAR_summary_subset$reef)
PAR_summary_subset$Season <- as.factor(PAR_summary_subset$Season)


treat_model <- lm(PAR_mol_day ~ reef*Season, data = PAR_summary_subset)
treat_model_metrics <- augment(treat_model)
plot(treat_model)


# Run the two-way ANOVA
result <- aov(PAR_mol_day ~ reef * Season, data = PAR_summary_subset)


# Display the summary of the ANOVA
summary(result)

capture.output(summary(result), file = "PAR_anova.csv")

TukeyHSD(aov(PAR_mol_day ~ reef * Season, data = PAR_summary_subset))
capture.output(TukeyHSD(aov(PAR_mol_day ~ reef * Season, data = PAR_summary_subset)), file = "PAR_anova_tukey.csv")
```
...
import bottle sample data and look at that to compare to in-situ (it looks like there is drift happening over time)
```{r}
filtered_data %>% 
  dplyr::select(Date:reef) %>% 
  mutate(type = "sensor") -> salinity_data_subset

bottlesampledata <- read_csv("raw_data/bottle-data.csv")

bottlesampledata %>% 
  dplyr::select(collected_date, collected_time, collected_site, calc_salinity) %>% 
  rename(Date = collected_date, Salinity = calc_salinity, reef = collected_site, Time = collected_time) %>% 
  mutate(Date = mdy(Date)) %>% 
  mutate(Date = as.character(Date)) %>% 
  mutate(Time = as.character(Time)) %>% 
  mutate(reef = case_when(reef == "Emerald Reef" ~ "Emerald", 
                          reef == "MacArthur Causeway North" ~ "MacNorth", 
                          reef == "MacArthur Cswy North" ~ "MacNorth", 
                          reef == "Mac North" ~ "MacNorth",
                          reef == "Emerald Ree" ~ "Emerald")) %>% 
  mutate(type = "bottle") %>% 
  full_join(., salinity_data_subset) %>% 
  group_by(reef, Date, type) %>% 
  mutate(mean_daily_salinity = mean(Salinity)) %>% 
  ggplot(., aes(x=Date, y=Salinity, color = type)) +
  facet_wrap(~reef, scales = "free_x") +
  geom_point(aes(alpha = type == "sensor")) +  # Adjust alpha based on group
  scale_alpha_manual(values = c(1, 0.1))
```

```{r}
filtered_data %>% 
  filter(Date < "2023-04-04" | Date > "2023-07-11") %>% 
  ggplot(., aes(x= as.Date(Date), y=Salinity, color = reef)) +
  theme_classic() +
  geom_point() +
  labs(x="Date", y = "Salinity") +
...
bottlesampledata %>% 
  ...
  filter(Date > "2022-11-16") -> salinity_bottlesampledata
...
# Calculate the range of dates (earliest and latest)
date_range <- range(as.Date(c("2022-11-17", "2023-03-28")))
date_range <- range(as.Date(c("2023-03-28", "2023-07-11")))
date_range <- range(as.Date(c("2023-07-11", "2023-11-20")))

# Find the midpoint date
midpoint <- mean(date_range) #2023-01-21, 2023-05-19, 2023-09-15

#then create data subsets, and use the bottle sample data for the nearest data range
#this is basically assuming that by the end of the deployment, the data will have drifted a lot, so you need to correct the later time points to the new bottle sample that was collected after

filtered_data %>% 
  dplyr::select(Date, Time, Salinity, reef, mean_daily_salinity) %>% 
  mutate(type = "CTD") %>% 
  mutate(Date = as.Date(Date)) -> salinity_CTD_data

salinity_CTD_data %>% 
  filter(Date < "2023-01-21") %>% 
  filter(reef == "Emerald") %>% 
  mutate(Salinity_offset = mean_daily_salinity - 35.78510) %>% 
  ungroup() %>% 
  summarise(mean_salinity_offset = mean(Salinity_offset)) #0.1646945





full_join(salinity_CTD_data,salinity_bottlesampledata) %>% 
  dplyr::select(!Time) %>% 
```
...
filtered_data %>% 
  group_by(reef, Date) %>% 
  mutate(mean_daily_salinity = mean(Salinity), sd_daily_salinity = sd(Salinity)) %>% 
  filter(Date < "2023-04-04" | Date > "2023-07-11") %>% 
  ggplot(., aes(x= as.Date(Date), y=mean_daily_salinity, color = reef)) +
  theme_classic() +
  geom_point() +
  geom_errorbar(aes(ymin = mean_daily_salinity - sd_daily_salinity, ymax = mean_daily_salinity + sd_daily_salinity), width = 0.2, alpha = 0.1) +
  labs(x="Date", y = "Salinity") +
  scale_color_manual(values = c("#edb9c3", "#5370c9")) + 
  theme(text = element_text(size = 15))
```
...
filtered_data %>% 
  ...
  mutate(mean_daily_salinity = mean(Salinity), sd_daily_salinity = sd(Salinity)) %>% 
  filter(Date < "2023-04-04" | Date > "2023-07-11") %>% 
  ggplot(., aes(x= reef, y=mean_daily_salinity, fill = reef)) +
  theme_classic() +
  geom_boxplot() +
  labs(x="Reef", y = "Salinity") +
  scale_fill_manual(values = c("#edb9c3", "#5370c9")) + 
  theme(text = element_text(size = 15))
```
...
filtered_data %>% 
  group_by(reef, Date) %>% 
  mutate(mean_daily_salinity = mean(Salinity), sd_daily_salinity = sd(Salinity)) %>% 
  filter(Date < "2023-04-04" | Date > "2023-07-11") -> salinity_subset

salinity_subset %>% 
  select(Date, reef, mean_daily_salinity) %>% 
  distinct() %>% 
  pivot_wider(names_from="reef", values_from = "mean_daily_salinity")-> mean_daily_salinity

wilcox.test(mean_daily_salinity$Emerald, mean_daily_salinity$MacNorth, alternative = "two.sided")
...
```{r}
filtered_data %>% 
  filter(Date < "2023-04-04" | Date > "2023-07-11") -> filtered_data

filtered_data %>% 
  mutate(Season = case_when(Date < "2023-05-26" ~ "Dry",
                            Date >= "2023-05-26" & Date <= "2023-10-15" ~ "Wet",
                            Date > "2023-10-15" ~ "Dry")) -> filtered_data_subset

library(rstatix)
# Ensure 'site' and 'season' are factors
filtered_data_subset$reef <- as.factor(filtered_data_subset$reef)
filtered_data_subset$Season <- as.factor(filtered_data_subset$Season)

filtered_data_subset %>% 
  mutate(salinity_sq = (Salinity)^2) -> filtered_data_subset

treat_model <- lm(salinity_sq ~ reef*Season, data = filtered_data_subset)
treat_model_metrics <- augment(treat_model)
plot(treat_model)

treat_model_metrics %>% 
  filter(abs(.std.resid) > 3) -> remove #126 rows to remove

anti_join(filtered_data_subset, remove) -> filtered_data_subset_removed


treat_model <- lm(salinity_sq ~ reef*Season, data = filtered_data_subset_removed)
treat_model_metrics <- augment(treat_model)
plot(treat_model)
#good enough

# Run the two-way ANOVA
result <- aov(salinity_sq ~ reef * Season, data = filtered_data_subset_removed)


# Display the summary of the ANOVA
summary(result)

capture.output(summary(result), file = "salinity_anova.csv")

TukeyHSD(aov(salinity_sq ~ reef * Season, data = filtered_data_subset_removed))
capture.output(TukeyHSD(aov(salinity_sq ~ reef * Season, data = filtered_data_subset_removed)), file = "salinity_anova_tukey.csv")
```
...
boxplot of raw pH sensor data
```{r}
pH_emerald %>% 
  select(`Datetime (EST)`, reef, `Internal pH (pH)`) %>% 
  rename(DateTime = `Datetime (EST)`) %>% 
  mutate(Date = as.Date(DateTime)) %>% 
  filter(`Internal pH (pH)` < 8.049 & `Internal pH (pH)` > 7.895) -> pH_emerald_subset

pH_macnorth %>% 
  select(`Datetime (EST)`, reef, `Internal pH (pH)`) %>% 
  rename(DateTime = `Datetime (EST)`) %>% 
  mutate(Date = as.Date(DateTime)) %>%
  filter(`Internal pH (pH)` < 8.142 & `Internal pH (pH)` > 7.619) -> ph_macnorth_subset

rbind(pH_emerald_subset, ph_macnorth_subset) -> all_ph_data_subset


all_ph_data_subset %>% 
  group_by(Date, reef) %>% 
  mutate(mean_daily_ph=mean(`Internal pH (pH)`)) %>% 
  select(!c(DateTime, `Internal pH (pH)`)) %>% 
  distinct() %>% 
  ggplot(., aes(x=reef, y=mean_daily_ph, fill=reef)) +
  geom_boxplot() +
  theme_classic() +
  scale_fill_manual(values = c("#edb9c3", "#5370c9")) + 
  theme(text = element_text(size = 15)) +
...
#set max and mins based on Enochs et al 2023
#emerald range: 7.895 - 8.049
#macnorth range: 7.619 - 8.142
```
...
all_ph_data_subset %>% 
  ...
  mutate(mean_daily_ph=mean(`Internal pH (pH)`)) %>% 
  select(!c(DateTime, `Internal pH (pH)`)) %>% 
  distinct() -> mean_daily_ph

treat_model <- lm(mean_daily_ph ~ reef, data = mean_daily_ph)
plot(treat_model) # not normal

mean_daily_ph %>% 
  pivot_wider(names_from="reef", values_from = "mean_daily_ph")-> mean_daily_ph

wilcox.test(mean_daily_ph$Emerald, mean_daily_ph$MacNorth, alternative = "two.sided")
...
```{r}
cowplot::plot_grid(parplot,salinity_plot,pH_plot, ncol=3)
ggsave("par_salinity_pH.pdf",width=15,height=7)
```
...
```{r}
tilt_emerald<- read_xlsx("raw_data/tilt-emerald-sn2102069-redacted.xlsx", sheet = "tilt-emerald-sn2102069")
tilt_macnorth<- read_xlsx("raw_data/tilt-macnorth-sn2102068-redacted.xlsx", sheet = "tilt-macnorth-sn2102068")
...
## stats
```{r}
rbind(tilt_emerald_clean, tilt_macnorth_clean) -> all_tilt_data

all_tilt_data %>% 
  mutate(Season = case_when(DateTime_EST < "2023-05-26 00:00:00" ~ "Dry",
                            DateTime_EST >= "2023-05-26 00:00:00" & DateTime_EST <= "2023-10-15 00:00:00" ~ "Wet",
                            DateTime_EST > "2023-10-15 00:00:00" ~ "Dry")) -> all_tilt_data

library(rstatix)
# Ensure 'site' and 'season' are factors
all_tilt_data$reef <- as.factor(all_tilt_data$reef)
all_tilt_data$Season <- as.factor(all_tilt_data$Season)


treat_model <- lm(ws ~ reef*Season, data = all_tilt_data)
treat_model_metrics <- augment(treat_model)
plot(treat_model)

treat_model_metrics %>% 
  filter(abs(.std.resid) > 3) -> remove #852 rows to remove

anti_join(all_tilt_data, remove) -> all_tilt_data_subset_removed


treat_model <- lm(ws ~ reef*Season, data = all_tilt_data_subset_removed)
treat_model_metrics <- augment(treat_model)
plot(treat_model)


all_tilt_data_subset_removed %>% 
  mutate(ws_sq=sqrt(ws)) -> all_tilt_data_subset_removed

treat_model <- lm(ws_sq ~ reef*Season, data = all_tilt_data_subset_removed)
treat_model_metrics <- augment(treat_model)
plot(treat_model)#good enough

# Run the two-way ANOVA
result <- aov(ws_sq ~ reef * Season, data = all_tilt_data_subset_removed)


# Display the summary of the ANOVA
summary(result)

capture.output(summary(result), file = "tilt_anova.csv")

TukeyHSD(aov(ws_sq ~ reef * Season, data = all_tilt_data_subset_removed))
capture.output(TukeyHSD(aov(ws_sq ~ reef * Season, data = all_tilt_data_subset_removed)), file = "tilt_anova_tukey.csv")
```



# temp + PAR + salinity + pH + current radar plot

```{r}
temp_summary # need to make one of these for each parameter 
```

PAR
```{r}
#we're missing a section of data for MacNorth, so make sure the datasets have the same date ranges so the extra data for one site isn't skewing it. then in results/figure caption, state the date range for each
...
rbind(ecopar_emerald_clean, ecopar_macnorth_clean) %>% 
  ...
  dplyr::rename(PAR_daytime=`PAR(avg)`) -> daytime_par_continuous # approx sunrise to sunset

daytime_par_continuous %>% 
  select(Date, reef) %>% 
  distinct() # MAcNorth data ends 2023-07-11

daytime_par_continuous %>% 
  filter(Date <= "2023-07-11") -> daytime_par_continuous

par.summary <- summaryBy(PAR_daytime ~ reef, data = daytime_par_continuous,
                         FUN = function(x) { c(max = max(x), min = min(x)) } )

par.summary$par.seasonal <- par.summary$PAR_daytime.max - par.summary$PAR_daytime.min

# Daily Range
Daily.range <- summaryBy(PAR_daytime ~ Date + reef, data=daytime_par_continuous, FUN=c(max,min,mean,sd))

Daily.range$par.daily.range <- Daily.range$PAR_daytime.max - Daily.range$PAR_daytime.min

par.d.range <- summaryBy(par.daily.range ~ reef, data = Daily.range,
                          FUN = function(x) { c(mean = mean(x)) } )

full_join(par.summary, par.d.range) -> par_summary
```
...
filtered_data %>% 
  select(Date, reef) %>% 
  distinct() # 2023-04-04 -- 2023-07-11 there is no MacN data

filtered_data %>% 
  filter(Date < "2023-04-04" | Date > "2023-07-11") -> filtered_data

salinity.summary <- summaryBy(Salinity ~ reef, data = filtered_data,
                         FUN = function(x) { c(max = max(x), min = min(x)) } )

salinity.summary$salinity.seasonal <- salinity.summary$Salinity.max - salinity.summary$Salinity.min

# Daily Range
Daily.range <- summaryBy(Salinity ~ Date + reef, data=filtered_data, FUN=c(max,min,mean,sd))

Daily.range$salinity.daily.range <- Daily.range$Salinity.max - Daily.range$Salinity.min

salinity.d.range <- summaryBy(salinity.daily.range ~ reef, data = Daily.range,
                          FUN = function(x) { c(mean = mean(x)) } )

full_join(salinity.summary, salinity.d.range) -> salinity_summary
```

pH (using raw internal pH because corrections with bottle samples were really off)
```{r}
all_ph_data %>% 
  dplyr::select(`Internal pH (pH)`, `Datetime (EST)`, reef) %>% 
  dplyr::rename(pH=`Internal pH (pH)`) %>% 
  separate(`Datetime (EST)`, into = c("Date", "Time"), sep = " ") -> ph_data

pH.summary <- summaryBy(pH ~ reef, data = ph_data,
                         FUN = function(x) { c(max = max(x), min = min(x)) } )

pH.summary$pH.seasonal <- pH.summary$pH.max - pH.summary$pH.min

# Daily Range
Daily.range <- summaryBy(pH ~ Date + reef, data=ph_data, FUN=c(max,min,mean,sd))

Daily.range$pH.daily.range <- Daily.range$pH.max - Daily.range$pH.min

pH.d.range <- summaryBy(pH.daily.range ~ reef, data = Daily.range,
                          FUN = function(x) { c(mean = mean(x)) } )

full_join(pH.summary, pH.d.range) -> pH_summary
```

current
```{r}
rbind(tilt_emerald_clean, tilt_macnorth_clean) %>% 
  dplyr::select(Date, ws, reef) %>% 
  drop_na(ws) -> tilt_data

tilt.summary <- summaryBy(ws ~ reef, data = tilt_data,
                         FUN = function(x) { c(max = max(x), min = min(x)) } )

tilt.summary$tilt.seasonal <- tilt.summary$ws.max - tilt.summary$ws.min

# Daily Range
Daily.range <- summaryBy(ws ~ Date + reef, data=tilt_data, FUN=c(max,min,mean,sd))

Daily.range$tilt.daily.range <- Daily.range$ws.max - Daily.range$ws.min

tilt.d.range <- summaryBy(tilt.daily.range ~ reef, data = Daily.range,
                          FUN = function(x) { c(mean = mean(x)) } )

full_join(tilt.summary, tilt.d.range) -> tilt_summary
```

## radar plot
```{r}
cbind(temp_summary_DHW, par_summary, salinity_summary, pH_summary, tilt_summary) %>% 
  column_to_rownames(var="reef") %>% 
  select(!reef) -> all_data

#write_csv(all_data, "radarplot_alldata.csv")
```
...
scale_by_max <- function(x) {
 ...
 all_data %>%
  dplyr::mutate(across(everything(), as.numeric)) %>%
  dplyr::mutate(across(everything(), scale_by_max)) -> all_data_percent

# To use the fmsb package, I have to add 2 lines to the dataframe: the max and min of each variable to show on the plot!
all_data_percent <- rbind(rep(100,5), rep(0,5), all_data_percent)
...
# plot with default options:
#pdf("radarchart_withDHW.pdf")
radarchart(all_data_percent, axistype=1, 
            #custom polygon
            pcol=colors_border , pfcol=colors_in , plwd=4 , plty=1,
            #custom the grid
            cglcol="grey", cglty=1, axislabcol="grey", caxislabels=seq(0,100,25), cglwd=0.8,
...
#this is very chaotic so let's take only the most interesting from each parameter

all_data_percent %>% 
  select(!ws.min) -> data_percent_subset

#pdf("radarchart_allenvdata.pdf")
radarchart(data_percent_subset, axistype=1, 
            #custom polygon
            pcol=colors_border , pfcol=colors_in , plwd=4 , plty=1,
            #custom the grid
            cglcol="grey", cglty=1, axislabcol="grey", caxislabels=seq(0,100,25), cglwd=0.8,
